...
public class AdmissionRepositoryCustomImpl implements AdmisisonRepositoryCustom {
    ...
    public Page<AdmissionEntity> searchAdmission(Map<String, Object> params, Pageable pageable) {
        ...
        String baseQuery = "SELECT a.* FROM admission a left join inpatient i ON i.inpatient_id =a.inpatient_id WHERE 1=1 and a.is_active = 1 and a.is_deleted = 0";
        String countQuery = "SELECT COUNT(*) FROM admission a inner join inpatient i ON i.inpatient_id =a.inpatient_id WHERE 1=1 and a.is_active = 1 and a.is_deleted = 0";
